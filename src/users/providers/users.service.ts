...
export class UsersService {
    ...
    async create(createUser: CreateUserRequest, avatar: Express.Multer.File): Promise<void> {
    ...
    async update(id: number, updateUser: UpdateUserRequest): Promise<UserResponse> {
        ...
        if (userToUpdate.password) {
            userToUpdate.password = await bcrypt.hash(userToUpdate.password, SALT_OR_ROUNDS);
        } else {
